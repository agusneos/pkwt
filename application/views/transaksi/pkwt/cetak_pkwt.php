...
$I          = $company.' , suatu perseroan terbatas yang didirikan menurut hukum Negara Republik Indonesia, berkedudukan di Jl. Raya Serang Cibarusah Rt 010/02 Cikarang 17550 dalam hal ini diwakili oleh Bapak Ir. Bambang Tjondro selaku Manager HRD-GA selaku kuasa dari Bapak Rudy Teo selaku Direktur Utama '.$company.' berdasarkan Surat Kuasa tertanggal 02 Januari 2013 oleh karenanya sah bertindak untuk dan atas nama '.$company.' (selanjutnya disebut sebagai PIHAK PERTAMA);';
...
$pasal2_1   = kwt2($data->pkwt_kk).' berlaku untuk jangka waktu '.st($data->pkwt_start, $data->pkwt_end).' ('.ucwords(Terbilang(st($data->pkwt_start, $data->pkwt_end))).') tahun terhitung mulai tanggal '.ft($data->pkwt_start).' sampai dengan tanggal '.ft($data->pkwt_end).' ("Tanggal Pengakhiran"), dengan batas maksimal waktu kontrak kerja sesuai perundangan-undangan ketenagakerjaan yang berlaku dan peraturan pelaksanaannya.';
...
$fpdf->Cell(0,0.5,$data->emply_name,0,0,'L');

$fpdf->Ln();
$fpdf->Cell(0.75,0.5,'',0,0,'L');
$fpdf->Cell(4,0.5,'Jenis kelamin',0,0,'L');
$fpdf->Cell(0.5,0.5,':',0,0,'C');
$fpdf->Cell(0,0.5,$data->emply_sex,0,0,'L');

$fpdf->Ln();
$fpdf->Cell(0.75,0.5,'',0,0,'L');
$fpdf->Cell(4,0.5,'Kawin / belum kawin',0,0,'L');
$fpdf->Cell(0.5,0.5,':',0,0,'C');
$fpdf->Cell(0,0.5,$data->emply_marital,0,0,'L');

$fpdf->Ln();
$fpdf->Cell(0.75,0.5,'',0,0,'L');
$fpdf->Cell(4,0.5,'Tempat / tgl lahir',0,0,'L');
$fpdf->Cell(0.5,0.5,':',0,0,'C');
$fpdf->Write(0.5,$data->emply_bop);
$fpdf->Write(0.5,' / ');
$fpdf->Write(0.5,ft($data->emply_bod));
...
$fpdf->Cell(0.5,0.5,':',0,0,'C');
$fpdf->Cell(0,0.5,$data->emply_ktp,0,0,'L');

$fpdf->Ln();
$fpdf->Cell(0.75,0.5,'',0,0,'L');
$fpdf->Cell(4,0.5,'Alamat',0,0,'L');
$fpdf->Cell(0.5,0.5,':',0,0,'C');
$fpdf->MultiCell(0,0.5,$data->emply_add,0,'J');
...
$fpdf->SetFont('Arial','',10);
$fpdf->Cell(0.75,0.5,'(1)',0,0,'L');
$fpdf->MultiCell(0,0.5,'Jam kerja di '.$company.' adalah 8 (delapan) jam sehari dan 40  (empat puluh) jam seminggu atau 5 (lima) hari kerja, apabila jam kerja melebihi ketentuan dihitung jam kerja lembur.',0,'J');
// PASAL 3-2
$fpdf->Ln(0.5);
$fpdf->Cell(0.75,0.5,'(2)',0,0,'L');
...
//Start Table
$fpdf->Ln(0.5);
$fpdf->BasicTable(array('Shift', 'Hari', 'Jam Kerja', 'Break Time', 'Istirahat'),$fpdf->LoadData(jadwal($data->workday_path)));
//End Table
...
$fpdf->Cell(2.75,0.5,'',0,0,'L');
$fpdf->Cell(0.75,0.5,'ii.',0,0,'L');
$fpdf->MultiCell(0,0.5,'PIHAK PERTAMA berkewajiban membayarkan upah lembur kepada PIHAK KEDUA sesuai ketentuan Keputusan Menteri Tenaga Kerja Dan Transmigrasi Republik Indonesia Nomor KEP.102/MEN/VI/2004 dan/atau ketentuan hukum lainnya.',0,'J');
...
$fpdf->Ln(0);
$fpdf->Cell(3.5,0.5,'',0,0,'L');
$fpdf->MultiCell(0,0.5,'Besarnya Tunjangan Hari Raya (THR) adalah 1 (satu) bulan upah pokok berikut tunjangan tetap apabila masa kerja PIHAK KEDUA 12 (dua belas) bulan atau lebih. Namun apabila masa kerja PIHAK KEDUA kurang dari 12 (dua belas) bulan tetapi sudah 3 (tiga) bulan atau lebih, maka Tunjangan Hari Raya (THR) akan diberikan secara proporsional.',0,'J');
...
$fpdf->Cell(0.75,0.5,'-',0,0,'L');
$fpdf->MultiCell(0,0.5,'PIHAK PERTAMA membayar tunjangan shift dengan jam kerja Shift 2 (dua)   kepada PIHAK KEDUA  sebesar Rp. 1.500 ( Seribu Lima Ratus Rupiah) per hadir.',0,'J');

$fpdf->Ln(0.5);
$fpdf->Cell(4,0.5,'',0,0,'L');
$fpdf->Cell(0.75,0.5,'-',0,0,'L');
$fpdf->MultiCell(0,0.5,'PIHAK PERTAMA membayar tunjangan shift dengan jam kerja Shift 3 (tiga)   kepada PIHAK KEDUA  sebesar Rp. 2.500 ( Dua Ribu Lima Ratus Rupiah) per hadir.',0,'J');
...
$fpdf->Ln(0);
$fpdf->Cell(3.5,0.5,'',0,0,'L');
$fpdf->MultiCell(0,0.5,'PIHAK PERTAMA membayar tunjangan transport sebesar Rp.6.000 (Enam Ribu Rupiah) per hadir kepada PIHAK KEDUA yang  tidak dapat menikmati  fasilitas yang telah disediakan oleh PIHAK PERTAMA sesuai dengan Peraturan Perusahaan pasal 23.',0,'J');

$fpdf->Ln(0.5);
$fpdf->Cell(2.75,0.5,'',0,0,'L');
$fpdf->Cell(0.75,0.5,'iii.',0,0,'L');
$fpdf->MultiCell(0,0.5,'PIHAK PERTAMA memberikan fasilitas Jaminan Sosial Tenaga Kerja (JAMSOSTEK) kepada PIHAK KEDUA sesuai dengan Undang-undang No. 3 tahun 1992 jo. Peraturan Pemerintah No. 14 tahun 1993.',0,'J');
...
$fpdf->Cell(0.75,0.5,'',0,0,'L');
$fpdf->Cell(0.75,0.5,'i.',0,0,'L');
$fpdf->MultiCell(0,0.5,'Terjadi peristiwa bencana alam, perang, kerusuhan sosial, gangguan keamanan yang mengakibatkan '.kwt2($data->pkwt_kk).' ini tidak mungkin lagi untuk dilanjutkan.',0,'J');
...
$fpdf->Ln(0.5);
$fpdf->Cell(9,0.5,'Nama: Ir. Bambang T.',0,0,'L');
$fpdf->Cell(9,0.5,'Nama: '.$data->emply_name,0,0,'L');

$fpdf->Ln(0.5);
$fpdf->Cell(9,0.5,'Jabatan: Manager HRD-GA',0,0,'L');
