    ...
    function transaksiPkwtHapus(){
        ...
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus data PKWT '+row.pkwt_id+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('transaksi/pkwt/delete'); ?>',{pkwt_id:row.pkwt_id},function(result){
                        if (result.success){
                            $('#grid-transaksi-pkwt').datagrid('reload');
                        } else {
                            $.messager.show({
...
<!-- Data Grid -->
<table id="grid-transaksi-pkwt" toolbar="#toolbar-transaksi-pkwt"
    ...
    <form id="fm-transaksi-pkwt" method="post" novalidate >
        ...
        <div class="fitem">
            ...
            <input id="nm" class="easyui-combobox" name="pkwt_nik" data-options="
                ...
                method:'get', valueField:'emply_nik', textField:'emply_name', panelHeight:'300'" />
        </div>
        <div class="fitem">
            <label for="type">Bagian</label>
            <input id="dp" class="easyui-combobox" name="pkwt_dept" data-options="
                url:'<?php echo site_url('transaksi/pkwt/getDept'); ?>',
                method:'get', valueField:'dept_id', textField:'dept_name', groupField:'dept_parent', panelHeight:'300'" />
        </div>
        <div class="fitem">
            <label for="type">Jabatan</label>
            <input id="ps" class="easyui-combobox" name="pkwt_post" data-options="
                url:'<?php echo site_url('transaksi/pkwt/getPost'); ?>',
                method:'get', valueField:'post_id', textField:'post_name', panelHeight:'auto'" />
        </div>
        <div class="fitem">
...
<script type="text/javascript">
    $('#grid-transaksi-pkwt').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('transaksi/pkwt/index'); ?>?grid=true'}).datagrid('enableFilter');
